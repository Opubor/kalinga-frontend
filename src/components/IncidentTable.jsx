...
function IncidentTable() {
  ...
  // SEARCH
  const search = (data) => {
    axios.get(`/reports?type=incident&q=${data}`).then((response) => {
      setIncidents(response.data);
      setCurrentPage(1);
    });
  };
  ...
  // SORT
  const sortAsc = () => {
    axios.get("/reports?type=incident&sortAsc=ascending").then((response) => {
      setIncidents(response?.data);
    });
  };
  const sortDsc = () => {
    axios.get("/reports?type=incident&sortAsc=descending").then((response) => {
      setIncidents(response?.data);
    });
  };
  ...
  const submitPicture = (event) => {
    ...
    axios
      .put(`/picturereport/${currentIncidentId}`, formData)
      .then((res) => {
        getIncidents();
  ...
  const submitFile = (event) => {
    ...
    axios
      .put(`/pdfreport/${currentIncidentId}`, formData)
      .then((res) => {
        getIncidents();
        toast.success(res.data);
      })
  ...
  const currentPosts = incidents.slice(indexOfFirstPost, indexOfLastPost);
  const pageCount = Math.ceil(incidents.length / postsPerPage);
  ...
  return (
    <>
      {/* ====================Table============================ */}
      <TablesLayout
        tableName={"Incidents Reports"}
        search={<SearchInput onSearch={search} />}
        pagination={
          <>
            <TotalNo totalnumber={incidents?.length} />
            <ReactPagination
              pageCount={pageCount}
            ...
            {currentPosts.map((data, index) => {
              return (
                <tr key={index}>
                  ...
                  <OptionsTd>
                    <ViewButton
                      viewFunction={`/view-incident?edit=${data?._id}`}
                    >
                      View
                    </ViewButton>
                    {facilityadmin && (
                      <>
                        <EditButton
                          editFunction={`/edit-incident?edit=${data?._id}`}
                        >
                          Edit
                        </EditButton>
                        <DeleteButton
